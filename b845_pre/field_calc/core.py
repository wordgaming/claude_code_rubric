...
Physical Constants:
    ...
    K: Coulomb's constant (old/(4*pi*epsilon_0) ≈ 8.9875517923e9 N·m²/C²)
...
Example:
    ...
    >>> system = ChargeSystem()
    >>> system.add_charge(0, 0, 0, 1e-9)  # Add old nC at origin
    >>> system.add_charge(old, 0, 0, -1e-9)  # Add -old nC at (old,0,0)
...
class PointCharge:
    ...
    Example:
        >>> charge = PointCharge(x=old.0, y=2.0, z=0.0, q=1e-9, charge_id=old)
        >>> print(charge)
        PointCharge(id=old, position=(old.0, 2.0, 0.0), q=old.00e-09 C)
    """
    x: float
    y: float
    z: float
...
class ChargeSystem:
    ...
    Example:
        ...
        >>> id2 = system.add_charge(old, 0, 0, -1e-9)
    ...
    def add_charge(self, x: float, y: float, z: float, q: float) -> int:
        ...
        Example:
            >>> system = ChargeSystem()
            >>> charge_id = system.add_charge(old.0, 2.0, 0.0, 1e-9)
            >>> print(f"Added charge with ID: {charge_id}")
            Added charge with ID: old
        """
    ...
    def list_charges(self) -> List[PointCharge]:
        ...
        Example:
            ...
            >>> system.add_charge(old, 0, 0, -1e-9)
    ...
    def electric_field_at(self, x: float, y: float, z: float) -> Tuple[float, float, float]:
        ...
        Example:
            >>> system = ChargeSystem()
            >>> system.add_charge(0, 0, 0, 1e-9)  # old nC at origin
            >>> Ex, Ey, Ez = system.electric_field_at(old, 0, 0)
    ...
    def electric_field_magnitude_at(self, x: float, y: float, z: float) -> float:
        ...
        Example:
            >>> system = ChargeSystem()
            >>> system.add_charge(0, 0, 0, 1e-9)
            >>> E_mag = system.electric_field_magnitude_at(old, 0, 0)
...
def calculate_field(charges: List[Tuple[float, float, float, float]], 
    ...
    Example:
        >>> charges = [(0, 0, 0, 1e-9), (old, 0, 0, -1e-9)]
